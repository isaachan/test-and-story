...
public class TestInformationsTest {
	...
	public void setUp() {
		infos = new TestInformations(new ArrayList<TestInformation>() {{
						add(new TestInformation(732, "test 1", true));
						add(new TestInformation(731, "test 2", true));
						add(new TestInformation(731, "test 3", true));
						add(new TestInformation(731, "test 4", true));
					}});
	}
	...
	public void should_get_all_sorted_test_informations_by_story_number() {
		...
		assertEquals("test 2", infosFor731.get(0).getMethodOrClassName());
		assertEquals("test 3", infosFor731.get(1).getMethodOrClassName());
